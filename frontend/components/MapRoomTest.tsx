...
const MapRoom: FC<MapRoomProps> = ({ roomId, username }) => {
  ...
  const [kafkaMessage, setKafkaMessage] = useState<OthersLocationType[]>([]);
    ...
    const fetchKafkaMessages = async () => {
      try {
        ...
        const response = await axios.get(
          `${BACKEND_URL}/api/get-kafka-messages/${roomId}`
        );
  ...
  const getUsersContainerHeight = () => {
    ...
    if (userCount === 0) return 180; // Height for empty state
    const baseHeight = 80; // Header height
    const userItemHeight = 72; // Height per user item
    const maxHeight = SCREEN_HEIGHT * 0.4; // Maximum 40% of screen height
          ...
          <BlurView intensity={20} tint="dark" style={styles.usersBlur}>
            <View style={styles.usersHeader}>
              ...
              <View style={styles.usersCount}>
                <Text style={styles.usersCountText}>{getActiveUsersCount()}</Text>
              </View>
            </View>
                    ...
                    <View style={styles.userLeft}>
                      ...
                      <View style={styles.userInfo}>
                        ...
                        <Text style={styles.userLocation} numberOfLines={1}>
                          {lastCoord.latitude.toFixed(4)}, {lastCoord.longitude.toFixed(4)}
                        </Text>
                      </View>
                    </View>
                    <View style={styles.userStatus}>
                      <View
                        style={[styles.statusDot, { backgroundColor: userColor }]}
                      />
...
const styles = StyleSheet.create({
  ...
  mapContainer: {
    ...
    shadowRadius: 20,
    elevation: 10,
    minHeight: 300, // Ensure map has minimum height
  },
  map: {
    width: "100%",
    height: "100%",
  ...
  usersContainer: {
    ...
    shadowRadius: 12,
    elevation: 8,
    minHeight: 80, // Minimum height for header only
  },
  usersBlur: {
    flex: 1,
    padding: 16,
